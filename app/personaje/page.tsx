...
import { loadCharacter, saveCharacter, type Item } from "@/lib/character";
...
export default function PersonajePage() {
    ...
    (async () => {
      ...
      if (!done && !cloud.current) {
        try {
          ...
          if (s) {
            const sheet = JSON.parse(s) as { level?: number; gold?: number; asi?: Asi; items?: Item[]; equipment?: Record<string, Item> };
  ...
  // Guardado de la ficha (nivel, oro, asi, items, equipo). No toca los campos de build.
  useEffect(() => {
    if (!loaded) return;
    const payload = { level, gold, asi, items, equipment };
    if (userId && cloud.current) {
      const t = setTimeout(() => saveCharacter(userId, payload), 700);
      return () => clearTimeout(t);
    }
    localStorage.setItem(SHEET_KEY, JSON.stringify(payload));
  }, [level, gold, asi, items, equipment, loaded, userId]);
